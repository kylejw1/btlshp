...
namespace Battleship
{
    public class Player
    {
        ...
        public Player(string name, IPlayerInterface commandProvider, IGameBoardView view)
        {
            Name = name;
        ...
        public void Fire(Player target)
        {
            ...
            while (!fireComplete)
            {
                ...
                if (target._ship.Points.Any(p => p.Equals(point)))
                {
                    cell.State = CellState.Hit;
                    target._view.SetHit(point);
                }
                else
                {
                    cell.State = CellState.Miss;
                    target._view.SetMiss(point);
                }
